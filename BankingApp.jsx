...
export default function BankingApp() {
  ...
  const accName      = (id) => state.accounts.find((x) => x.id === id)?.nome ?? "—";
  ...
  return (
    <div style={{ fontFamily: "var(--font-sans)", padding: "1.5rem 1rem", maxWidth: 900, margin: "0 auto", position: "relative" }}>
      {toast && (
        <div style={{ position: "absolute", top: 0, right: 0, zIndex: 50, padding: "10px 16px", borderRadius: "var(--border-radius-md)", fontSize: 13, fontWeight: 500, background: toast.type === "error" ? "var(--color-background-danger)" : "var(--color-background-success)", color: toast.type === "error" ? "var(--color-text-danger)" : "var(--color-text-success)", border: "0.5px solid", borderColor: toast.type === "error" ? "var(--color-border-danger)" : "var(--color-border-success)" }}>
          {toast.msg}
        </div>
      )}
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: "1.5rem" }}>
        ...
        <div>
          <p style={{ margin: 0, fontWeight: 500, fontSize: 16 }}>BancoApp</p>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: "1.5rem", borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
        {TABS.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ padding: "8px 14px", fontSize: 13, background: "transparent", border: "none", cursor: "pointer", fontWeight: tab === t.id ? 500 : 400, color: tab === t.id ? "var(--color-text-primary)" : "var(--color-text-secondary)", borderBottom: tab === t.id ? "2px solid var(--color-text-primary)" : "2px solid transparent", marginBottom: -1, borderRadius: 0 }}>{t.label}</button>
        ))}
      </div>
      ...
      {tab === "dashboard" && (
        <div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: "1.5rem" }}>
            {[{ label: "Patrimônio total", value: fmt(totalBalance) }, { label: "Contas ativas", value: active.length }, { label: "Transações", value: state.transactions.length }, { label: "Última operação", value: state.transactions[0] ? fmt(state.transactions[0].valor) : "—" }].map((c) => (
            ...
            {state.transactions.slice(0, 6).map((txn, i, arr) => (
              <div key={txn.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: i < arr.length - 1 ? "0.5px solid var(--color-border-tertiary)" : "none", background: "var(--color-background-primary)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 28, height: 28, borderRadius: "50%", background: `var(--color-background-${TIPO_COLOR[txn.tipo]})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 500, color: `var(--color-text-${TIPO_COLOR[txn.tipo]})` }}>{txn.tipo === "deposito" ? "D" : txn.tipo === "saque" ? "S" : "T"}</div>
                  <div>
                    <p style={{ margin: 0, fontSize: 13, fontWeight: 500 }}>{txn.descricao || TIPO_LABEL[txn.tipo]}</p>
                    <p style={{ margin: 0, fontSize: 11, color: "var(--color-text-secondary)" }}>{txn.tipo === "transferencia" ? `${accName(txn.origem)} → ${accName(txn.destino)}` : accName(txn.destino || txn.origem)}</p>
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <p style={{ margin: 0, fontSize: 13, fontWeight: 500, color: txn.tipo === "deposito" ? "var(--color-text-success)" : txn.tipo === "saque" ? "var(--color-text-danger)" : "var(--color-text-primary)" }}>{txn.tipo === "deposito" ? "+" : txn.tipo === "saque" ? "-" : ""}{fmt(txn.valor)}</p>
            ...
            {active.map((acc, i) => (
              <div key={acc.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", borderBottom: i < active.length - 1 ? "0.5px solid var(--color-border-tertiary)" : "none", background: "var(--color-background-primary)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 36, height: 36, borderRadius: "50%", background: "var(--color-background-info)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 500, color: "var(--color-text-info)" }}>{acc.nome.slice(0, 2).toUpperCase()}</div>
                  <div>
        ...
        <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10, marginBottom: 12 }}>
              <div>
                ...
                <select value={txnForm.tipo} onChange={(e) => setTxnForm({ ...BLANK_TXN, tipo: e.target.value })} style={{ width: "100%" }}>
                  <option value="deposito">Depósito</option><option value="saque">Saque</option><option value="transferencia">Transferência</option>
                </select>
              </div>
              <div>
          ...
          <div style={{ border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", overflow: "hidden" }}>
            {state.transactions.map((txn, i, arr) => (
              <div key={txn.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: i < arr.length - 1 ? "0.5px solid var(--color-border-tertiary)" : "none", background: "var(--color-background-primary)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 11, padding: "3px 8px", borderRadius: "var(--border-radius-md)", fontWeight: 500, background: `var(--color-background-${TIPO_COLOR[txn.tipo]})`, color: `var(--color-text-${TIPO_COLOR[txn.tipo]})` }}>{TIPO_LABEL[txn.tipo]}</span>
                  <div>
                    <p style={{ margin: 0, fontSize: 13 }}>{txn.descricao || "—"}</p>
                    <p style={{ margin: 0, fontSize: 11, color: "var(--color-text-secondary)" }}>{txn.tipo === "transferencia" ? `${accName(txn.origem)} → ${accName(txn.destino)}` : txn.tipo === "deposito" ? `→ ${accName(txn.destino)}` : `← ${accName(txn.origem)}`}</p>
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
      ...
      {tab === "testes" && (
        <div>
          <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-lg)", padding: "1.25rem", marginBottom: "1.5rem" }}>
            ...
            <p style={{ margin: "0 0 16px", fontSize: 13, color: "var(--color-text-secondary)" }}>12 testes automatizados verificam integridade do banco, validação de campos, unicidade de IDs, consistência de transações e regras de negócio.</p>
            <button onClick={() => setTestRes(runTests(state))} style={{ fontSize: 13, padding: "8px 16px" }}>▶ Executar todos os testes</button>
          </div>
          {testRes && (
            <div>
              <div style={{ display: "flex", gap: 10, marginBottom: "1rem" }}>
                <div style={{ background: "var(--color-background-success)", borderRadius: "var(--border-radius-md)", padding: "8px 14px" }}><span style={{ fontSize: 13, fontWeight: 500, color: "var(--color-text-success)" }}>{testRes.filter((t) => t.pass).length} passaram</span></div>
                <div style={{ background: "var(--color-background-danger)",  borderRadius: "var(--border-radius-md)", padding: "8px 14px" }}><span style={{ fontSize: 13, fontWeight: 500, color: "var(--color-text-danger)"  }}>{testRes.filter((t) => !t.pass).length} falharam</span></div>
                <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", padding: "8px 14px" }}><span style={{ fontSize: 13, color: "var(--color-text-secondary)" }}>{testRes.length} total</span></div>
              </div>
              <div style={{ border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", overflow: "hidden" }}>
                {testRes.map((t, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 16px", borderBottom: i < testRes.length - 1 ? "0.5px solid var(--color-border-tertiary)" : "none", background: "var(--color-background-primary)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 20, height: 20, borderRadius: "50%", background: t.pass ? "var(--color-background-success)" : "var(--color-background-danger)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: t.pass ? "var(--color-text-success)" : "var(--color-text-danger)" }}>{t.pass ? "✓" : "✗"}</div>
                      <p style={{ margin: 0, fontSize: 13 }}>{t.name}</p>
                    </div>
                    <span style={{ fontSize: 11, padding: "3px 8px", borderRadius: "var(--border-radius-md)", background: t.pass ? "var(--color-background-success)" : "var(--color-background-danger)", color: t.pass ? "var(--color-text-success)" : "var(--color-text-danger)" }}>{t.msg}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {modal && (
        <div onClick={() => setModal(null)} style={{ position: "absolute", inset: 0, minHeight: "100%", background: "rgba(0,0,0,0.4)", zIndex: 100, display: "flex", alignItems: "flex-start", justifyContent: "center", paddingTop: 80 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "1.5rem", width: "100%", maxWidth: 420 }}>
            {modal.type === "create" && (
              <>
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <select value={accForm.tipo} onChange={(e) => setAccForm({ ...accForm, tipo: e.target.value })} style={{ width: "100%" }}><option value="corrente">Corrente</option><option value="poupança">Poupança</option><option value="salário">Salário</option></select>
                </div>
            ...
            {modal.type === "edit" && (
              <>
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ fontSize: 12, color: "var(--color-text-secondary)", display: "block", marginBottom: 4 }}>Tipo de conta</label>
                  <select value={modal.data.tipo} onChange={(e) => setModal({ ...modal, data: { ...modal.data, tipo: e.target.value } })} style={{ width: "100%" }}><option value="corrente">Corrente</option><option value="poupança">Poupança</option><option value="salário">Salário</option></select>
                </div>
                <div style={{ display: "flex", gap: 8 }}>
            ...
            {modal.type === "delete" && (
              <>
                ...
                <p style={{ margin: "0 0 1.5rem", fontSize: 13, color: "var(--color-text-secondary)" }}>A conta de <strong>{modal.data.nome}</strong> será removida permanentemente. Esta ação não pode ser desfeita.</p>
                <div style={{ display: "flex", gap: 8 }}>
